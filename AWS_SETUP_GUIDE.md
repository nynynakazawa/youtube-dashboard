...
**重要**: Lambda関数で外部パッケージ（`pydantic`、`boto3`、`requests`、`pymysql`など）を使用するため、依存関係をZIPファイルに含める必要があります。

```bash
cd backend

# 依存関係を現在のディレクトリにインストール（Lambda用）
pip install -t . -r requirements.txt

# ZIPファイルを作成（依存関係とコードを含める）
zip -r lambda-functions.zip handlers/ services/ db/ common/ utils/ constants/ \
    boto3* requests* pydantic* pymysql* -x "*.pyc" "__pycache__/*" "*.dist-info/*" "*/tests/*" "*/test/*"

# インストールした依存関係をクリーンアップ（オプション）
rm -rf boto3* requests* pydantic* pymysql* *.dist-info
```

**より確実な方法（推奨）**: 一時ディレクトリを使用して依存関係をインストールする方法：
...
# ZIPファイルを作成
zip -r ../lambda-functions.zip . -x "*.pyc" "__pycache__/*" "*.dist-info/*" "*/tests/*" "*/test/*"
...
**注意**: 
- ZIPファイル名は任意です（例: `lambda-functions.zip`、`youtube-dashboard-backend.zip`など）。すべてのLambda関数で同じZIPファイルを使用します。
- ZIPファイルのサイズが250MBを超える場合は、Lambda Layerの使用を検討してください。
...
**重要**: このセクションでは、**無料で動作させる方法**と**セキュアな方法（有料）**の2つの選択肢を説明します。

#### 4.5.0 無料で動作させる方法 vs セキュアな方法（有料）

##### 方法1: Lambda関数をVPC外に配置（無料、開発環境向け）

- **メリット**:
  - **無料**: NATゲートウェイが不要
  - インターネット（YouTube API）に直接アクセス可能
  - DynamoDBに直接アクセス可能（VPCエンドポイント不要）
- **デメリット**:
  - RDSのパブリックアクセスを有効にする必要がある（セキュリティリスク）
  - 本番環境には不向き
- **必要な設定**:
  1. Lambda関数をVPC外に配置（VPC設定を削除）
  2. RDSのパブリックアクセスを有効にする
  3. セキュリティグループで特定のIPアドレスからのみアクセスを許可（推奨）

##### 方法2: Lambda関数をVPC内に配置 + NATゲートウェイ（有料、本番環境向け）

- **メリット**:
  - セキュア: RDSをインターネットから完全に隔離
  - 本番環境に適している
- **デメリット**:
  - **有料**: NATゲートウェイが必要（約$0.045/時間 + データ転送料金）
  - 設定が複雑
- **必要な設定**:
  1. Lambda関数をVPC内に配置
  2. NATゲートウェイを作成（有料）
  3. DynamoDB VPCエンドポイントを作成（無料）

**推奨**: 開発環境では**方法1（無料）**、本番環境では**方法2（有料）**を推奨します。

#### 4.5.1 VPCとは

**VPC（Virtual Private Cloud）**は、AWS上に構築する**仮想的なプライベートネットワーク**です。

##### VPCの基本概念

- **プライベートネットワーク**: インターネットから隔離された、あなた専用のネットワーク空間
- **論理的な分離**: 他のAWSアカウントやユーザーから完全に分離された環境
- **カスタマイズ可能**: IPアドレス範囲、サブネット、ルーティング、セキュリティグループなどを自由に設定可能

##### VPCの構成要素

1. **VPC（Virtual Private Cloud）**
   - プライベートネットワークのコンテナ
   - IPアドレス範囲（CIDRブロック）を定義（例: `10.0.0.0/16`）

2. **サブネット（Subnet）**
   - VPC内の小さなネットワーク領域
   - 可用性ゾーン（AZ）ごとに作成可能
   - 例: `10.0.1.0/24`（東京リージョンのAZ-1）、`10.0.2.0/24`（東京リージョンのAZ-2）

3. **ルートテーブル（Route Table）**
   - ネットワークトラフィックの経路を制御
   - どのサブネットからどこに通信できるかを定義

4. **セキュリティグループ（Security Group）**
   - 仮想的なファイアウォール
   - インバウンド（受信）とアウトバウンド（送信）のトラフィックを制御
   - 例: RDSへのアクセスを特定のセキュリティグループからのみ許可

5. **インターネットゲートウェイ（Internet Gateway）**
   - VPCとインターネットを接続するゲートウェイ
   - パブリックサブネットからインターネットにアクセス可能にする

6. **NATゲートウェイ（NAT Gateway）**
   - プライベートサブネットからインターネットにアクセスするためのゲートウェイ
   - プライベートサブネット内のリソースがインターネットに接続できるようにする

##### なぜVPCが必要なのか

1. **セキュリティ**: データベース（RDS）などの重要なリソースをインターネットから隔離
2. **ネットワーク制御**: どのリソースがどのリソースにアクセスできるかを細かく制御
3. **コンプライアンス**: 企業のセキュリティポリシーや規制要件を満たすため

##### このプロジェクトでのVPCの使用目的

- **RDS（MySQL）へのアクセス**: Lambda関数からRDSに安全に接続
- **ネットワーク分離**: RDSをインターネットから完全に隔離
- **セキュリティ**: セキュリティグループでアクセスを制御

##### Default VPC vs カスタムVPC

- **Default VPC**: AWSアカウント作成時に自動的に作成されるVPC
  - 開発環境や小規模なプロジェクトに適している
  - このプロジェクトではDefault VPCで問題なし
- **カスタムVPC**: 自分で作成するVPC
  - 本番環境や大規模なプロジェクトに適している
  - より細かいネットワーク制御が可能

#### 4.5.2 方法1: Lambda関数をVPC外に配置（無料、開発環境向け）

この方法では、Lambda関数をVPC外に配置し、RDSのパブリックアクセスを有効にします。**無料で動作させることができます**。

##### 4.5.2.1 Lambda関数のVPC設定を削除

1. Lambda関数の**設定**タブで**VPC**を開く
2. **編集**をクリック
3. **VPC設定を削除**:
   - すべてのRDSにアクセスできなくな（VPC、サブネット、セキュリティグループを空にする）
4. **保存**をクリック

**注意**: Lambda関数をVPC外に配置すると、RDSに直接アクセスできなくなります。次の手順でRDSのパブリックアクセスを有効にする必要があります。

##### 4.5.2.2 RDSのパブリックアクセスを有効にする

1. **RDSコンソール**でデータベースを選択
2. **接続とセキュリティ**タブを開く
3. **パブリックアクセスの設定**を確認:
   - **パブリックアクセス可能**: **はい**に変更（現在「いいえ」の場合）
4. **変更の保存**をクリック
5. データベースの**変更の適用**を待つ（数分かかります）

**重要**: RDSのパブリックアクセスを有効にすると、インターネットからアクセス可能になります。セキュリティグループで適切に制限してください。

##### 4.5.2.3 セキュリティグループでアクセスを制限（推奨）

セキュリティ上の理由から、特定のIPアドレスからのみアクセスを許可することを推奨します。
...
7. 以下の設定を行う：
   ...
   - **ソース**: カスタム
   - **IPアドレス**: あなたのIPアドレス（例: `123.45.67.89/32`）
     - または、すべてのIPアドレスを許可する場合は `0.0.0.0/0`（非推奨、開発環境のみ）
8. **ルールを保存**をクリック

**注意**: `0.0.0.0/0`を許可すると、インターネット上の誰でもRDSにアクセスできるようになります。開発環境のみで使用してください。

##### 4.5.2.4 Lambda関数の環境変数を確認

Lambda関数の環境変数`DB_HOST`が、RDSのパブリックエンドポイントを指していることを確認してください。

1. Lambdaコンソールで関数を開く
2. **設定**タブ → **環境変数**を開く
3. `DB_HOST`が正しいエンドポイントを指しているか確認
   - 例: `youtube-dashboard-db.xxxxx.ap-northeast-1.rds.amazonaws.com`

##### 4.5.2.5 動作確認

1. Lambda関数を実行して、RDSへの接続が成功するか確認
2. CloudWatch Logsでエラーが発生していないか確認
...
**デメリット**:
- RDSがインターネットに公開される（セキュリティリスク）
- 本番環境には不向き

#### 4.5.3 方法2: Lambda関数をVPC内に配置（有料、本番環境向け）

この方法では、Lambda関数をVPC内に配置し、NATゲートウェイを使用してインターネットにアクセスします。
...
**重要**: 
- VPC内のLambda関数からDynamoDBにアクセスする場合は、セクション4.6でVPCエンドポイントを設定する必要があります。
- VPC内のLambda関数からインターネット（YouTube API）にアクセスするには、NATゲートウェイが必要です（有料、約$0.045/時間）。

##### 4.5.3.2 NATゲートウェイの作成（有料）

NATゲートウェイを作成すると、VPC内のLambda関数からインターネットにアクセスできるようになります。

**注意**: NATゲートウェイは**有料**です（約$0.045/時間 + データ転送料金）。開発環境では、方法1（VPC外配置）を推奨します。

NATゲートウェイの作成手順は、AWS公式ドキュメントを参照してください：
- [NATゲートウェイの作成](https://docs.aws.amazon.com/ja_jp/vpc/latest/userguide/vpc-nat-gateway.html)

### 4.6 DynamoDB VPCエンドポイントの設定（VPC内のLambdaからDynamoDBにアクセスする場合）

VPC内に配置されたLambda関数からDynamoDBにアクセスするには、**VPCエンドポイント**を作成する必要があります。VPCエンドポイントがない場合、Lambda関数はDynamoDBに接続できず、タイムアウトエラーが発生します。

#### 4.6.1 VPCエンドポイントとは

- **VPCエンドポイント**: VPC内のリソースからAWSサービス（DynamoDB、S3など）にプライベート接続を提供する機能
- **Gateway型エンドポイント**: S3、DynamoDB専用（無料）
- **Interface型エンドポイント**: その他のAWSサービス用（有料、時間課金）

DynamoDBは**Gateway型エンドポイント**を使用するため、**追加費用はかかりません**。

#### 4.6.2 VPCエンドポイントの作成手順（GUI）

1. **AWSコンソールでVPCサービスを開く**
   - AWSコンソールの検索バーで「VPC」と入力
   - **VPC**サービスを選択

2. **左側のメニューから「エンドポイント」を選択**
   - 左側のメニューで**エンドポイント**をクリック

3. **「エンドポイントの作成」ボタンをクリック**

4. **エンドポイントの設定を行う**:
   - **名前タグ**: `dynamodb-endpoint`（任意の名前）
   - **サービスカテゴリ**: **AWSのサービス**を選択
   - **サービス**: 検索バーに「DynamoDB」と入力すると、以下の3つのサービスが表示されます：
     - **com.amazonaws.ap-northeast-1.dynamodb**（Gateway型）← **これを選択**
     - **com.amazonaws.ap-northeast-1.dynamodb**（Gateway型）- 2つ目
     - **com.amazonaws.ap-northeast-1.dynamodb**（Interface型）
     
     **重要**: **Gateway型**を選択してください。理由：
     - **Gateway型**: 無料、DynamoDB専用、高速
     - **Interface型**: 有料（時間課金）、汎用的だが高コスト
     
     **選択方法**:
     - サービス名の横に「**Gateway**」と表示されているものを選択
     - または、サービス名をクリックして詳細を確認し、「エンドポイントタイプ」が「Gateway」になっているものを選択
     - **リージョン（`ap-northeast-1`）が正しいことを確認**
   - **VPC**: Lambda関数が使用しているVPCを選択
     - Lambda関数のVPC IDを確認する方法：
       1. Lambdaコンソールで関数を開く
       2. **設定**タブ → **VPC**を開く
       3. VPC IDを確認（例: `vpc-0ff55ac0b0012e56f`）

5. **ルートテーブルを選択**:
   - **ルートテーブル**: Lambda関数が使用しているサブネットに関連付けられたルートテーブルを選択
     - 複数のルートテーブルがある場合は、すべて選択
     - Lambda関数のサブネットIDを確認する方法：
       1. Lambdaコンソールで関数を開く
       2. **設定**タブ → **VPC**を開く
       3. サブネットIDを確認（例: `subnet-0141f925c4b482f8d`）
     - ルートテーブルを確認する方法：
       1. VPCコンソールで**ルートテーブル**を開く
       2. Lambda関数のサブネットに関連付けられたルートテーブルを確認

6. **ポリシーを設定**（オプション）:
   - **ポリシー**: **フルアクセス**を選択（デフォルト）
     - より厳密なアクセス制御が必要な場合は、カスタムポリシーを設定可能

7. **「エンドポイントの作成」ボタンをクリック**

8. **エンドポイントの作成を待つ**:
   - ステータスが**利用可能**になるまで数分かかる場合があります
   - ステータスが**利用可能**になれば、VPC内のLambda関数からDynamoDBにアクセスできるようになります

#### 4.6.3 確認方法

1. **VPCコンソールでエンドポイントの状態を確認**:
   - **エンドポイント**ページで、作成したエンドポイントの**状態**が**利用可能**になっていることを確認

2. **Lambda関数からDynamoDBにアクセスできるか確認**:
   - Lambda関数を実行して、DynamoDBへの接続が成功するか確認
   - CloudWatch Logsでエラーが発生していないか確認
...
- **Lambda関数からDynamoDBにアクセスできない**:
  - エンドポイントの状態が「利用可能」になっているか確認
  - Lambda関数のVPC設定が正しいか確認
  - IAMロールにDynamoDBへのアクセス権限があるか確認

- **コストについて**:
  - DynamoDBのGateway型エンドポイントは**無料**です
  - データ転送料金も発生しません

#### 4.6.5 Gateway型とInterface型の違い
...
- **コスト**: **無料**（追加費用なし）
- **設定**: ルートテーブルにルートを追加するだけ
- **パフォーマンス**: 高速
- **このプロジェクトでの使用**: DynamoDBへのアクセスに使用

##### Interface型エンドポイント

- **対応サービス**: その他のAWSサービス（Lambda、SNS、SQSなど）
- **コスト**: **有料**（時間課金、約$0.01/時間）
- **設定**: ENI（Elastic Network Interface）を作成
- **パフォーマンス**: 高速だが、Gateway型より若干遅い場合がある
- **このプロジェクトでの使用**: 不要（DynamoDBはGateway型で対応可能）

##### どちらを選ぶべきか

- **DynamoDBへのアクセス**: **Gateway型**を選択（無料で高速）
- **その他のAWSサービス**: Interface型が必要な場合のみ使用（有料）

**重要**: DynamoDBの場合は必ず**Gateway型**を選択してください。Interface型を選択すると、不要なコストが発生します。

#### 4.6.6 参考情報

- **このプロジェクトでの使用目的**:
  - DynamoDBテーブル（`channel_update_cache`）へのアクセス
  - YouTube APIのレート制限を避けるためのキャッシュ機能

### 4.7 他のLambda関数の作成

同様の手順で、以下のLambda関数も作成してください。**すべて同じZIPファイル（`lambda-functions.zip`）を使用します**。各Lambda関数では、ハンドラーのみを変更します。

#### 4.7.1 `youtube-dashboard-list-channels`の作成
...
#### 4.7.2 `youtube-dashboard-get-channel-detail`の作成
...
#### 4.7.3 `youtube-dashboard-get-channel-videos`の作成
...
3. **Lambda関数で「No module named 'pydantic'」などのモジュールエラーが発生する**
   - **原因**: Lambda関数のZIPファイルに依存関係（外部パッケージ）が含まれていない
   - **エラーメッセージ例**: 
     - `Runtime.ImportModuleError: Unable to import module 'handlers.list_channels': No module named 'pydantic'`
     - `No module named 'boto3'`
     - `No module named 'requests'`
   - **解決方法**:
     1. セクション4.2の手順に従って、依存関係を含むZIPファイルを再作成してください
     2. 一時ディレクトリを使用する方法（推奨）を実行：
        ```bash
        cd backend
        mkdir -p lambda-package
        cd lambda-package
        ...
        rm -rf lambda-package
        ```
     3. 作成したZIPファイルをすべてのLambda関数に再アップロードしてください
   - **注意**: 依存関係を含むZIPファイルは、すべてのLambda関数で同じものを使用できます

3-1. **Lambda関数で「No module named 'pydantic'」エラーが発生する（macOSでZIPを作成した場合）**
   - **原因**: macOSで作成したZIPファイルにmacOS用のバイナリ（`.darwin.so`）が含まれており、LambdaのLinux環境では動作しない
   - **エラーメッセージ例**: 
     - `Runtime.ImportModuleError: Unable to import module 'handlers.list_channels': No module named 'pydantic'`
     - `ModuleNotFoundError: No module named 'pydantic_core'`
   - **確認方法**: ZIPファイル内にmacOS用バイナリが含まれているか確認：
     ```bash
     unzip -l lambda-functions.zip | grep "darwin\|macos" -i
     ```
   - **解決方法（Dockerを使用）**: Lambdaと同じLinux環境で依存関係をインストール：
     ```bash
     cd backend
     rm -rf lambda-package
     mkdir -p lambda-package
     
     # DockerでLambdaと同じ環境（Amazon Linux 2）を使用
     docker run --rm \
       -v "$(pwd):/var/task" \
       -w /var/task \
       public.ecr.aws/lambda/python:3.11 \
       bash -c "pip install -r requirements.txt -t lambda-package/ && \
                cp -r handlers services db common utils constants lambda-package/ && \
                cd lambda-package && \
                zip -r ../lambda-functions.zip . -x '*.pyc' '__pycache__/*' '*.dist-info/*' '*/tests/*' '*/test/*'"
     
     rm -rf lambda-package
     ```
   - **解決方法（Dockerが使えない場合）**: Linux環境（EC2、GitHub Actions、CI/CD環境など）でZIPファイルを作成する
   - **注意**: macOSで作成したZIPファイルは、バイナリ依存パッケージ（`pydantic-core`、`charset-normalizer`など）がLinux環境で動作しない可能性があります
...
6. **DynamoDBにアクセスできない**
   ...
   - テーブル名が正しいか確認
   - **VPC内のLambda関数からDynamoDBにアクセスする場合**: VPCエンドポイントが必要（セクション4.6を参照）
   - VPCエンドポイントが設定されていない場合、Lambda関数はDynamoDBに接続できず、タイムアウトエラーが発生します
